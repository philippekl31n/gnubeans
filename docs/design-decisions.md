...
- Collision after sanitization — resolved: detected during parse; both originals and the conflicting currency named in a stderr warning; all involved `Commodity` instances tagged `collision=True`; plan YAML marks colliding entries with an inline `COLLISION` comment; renderer raises `ValueError` if confirmed symbols still collide. In interactive mode, after the initial batch-decision table the tool checks for collisions in the confirmed symbols and re-prompts for each offending row in a loop until all are resolved — the user cannot exit the prompt with an unresolved collision.
